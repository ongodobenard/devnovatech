...
.dn-tc:hover{transform:translateY(-7px) scale(1.01);box-shadow:0 20px 48px rgba(0,200,204,0.13)}
.dn-tr{transition:background .18s ease,padding-left .18s ease;border-radius:6px}
.dn-tr:hover{background:rgba(0,200,204,0.05);padding-left:4px}
...
.dn-code-col{position:absolute;top:100%;font-family:monospace;font-size:11px;color:#00C8CC;pointer-events:none;user-select:none;white-space:nowrap;will-change:transform,opacity}
.dn-code-col:nth-child(1){left:4%;animation:dnCodeDrift 18s linear 0s infinite}
.dn-code-col:nth-child(2){left:22%;animation:dnCodeDrift 22s linear 3s infinite}
.dn-code-col:nth-child(3){left:48%;animation:dnCodeDrift 20s linear 7s infinite}
.dn-code-col:nth-child(4){left:72%;animation:dnCodeDrift 25s linear 1s infinite}
.dn-code-col:nth-child(5){left:88%;animation:dnCodeDrift 19s linear 5s infinite}
...
/* ─── Coding background (works on both dark navy and white sections) ── */
const CODE_SNIPPETS = [
...
function CodingBackground({ light = false }) {
  const color = light ? 'rgba(0,200,204,0.07)' : 'rgba(0,200,204,0.18)'
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none" style={{ zIndex: 0 }}>
      {CODE_SNIPPETS.map((col, i) => (
        <div key={i} className="dn-code-col" style={{ color }}>
...
function CircuitCard() {
  return (
    <svg className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 400 400" preserveAspectRatio="xMidYMid slice" style={{ opacity: 0.18 }}>
      <defs>
...
const WHY_US = [
  { num: '01', title: 'Nairobi-Based & Kenya-Focused',  desc: 'We understand the Kenyan market deeply, M-Pesa, local SEO, Safaricom APIs, low-bandwidth optimization and Kenyan user behaviour.' },
  { num: '02', title: 'Transparent KSh Pricing',        desc: 'No hidden fees, no dollar pricing surprises. Every project comes with a clear KSh proposal itemizing exactly what you get.' },
  { num: '03', title: 'Fast Delivery, Real Deadlines',  desc: 'Most websites are delivered in 7–14 days. We set realistic timelines and stick to them, keeping you updated on WhatsApp.' },
  { num: '04', title: 'SEO Built In From Day One',      desc: 'Every website we build includes on-page SEO, Google Search Console setup and proper meta tags from the start.' },
  { num: '05', title: 'M-Pesa & Kenya Payment Experts', desc: 'We have integrated M-Pesa STK Push, Airtel Money and card payments into dozens of Kenyan sites and apps.' },
  { num: '06', title: 'Post-Launch Support Included',   desc: 'Every project includes post-launch support. We do not disappear after delivery, we train your team and fix issues.' },
]

const TECH_CATS = [
  {
    category: 'Frontend & Web', color: '#00C8CC', floatCls: 'dn-f0', Icon: IconCode,
    tools: [
  ...
  {
    category: 'Backend & Databases', color: '#E8332A', floatCls: 'dn-f1', Icon: IconDatabase,
    tools: [
  ...
  {
    category: 'Mobile & Android', color: '#22c55e', floatCls: 'dn-f2', Icon: IconPhone,
    tools: [
  ...
  {
    category: 'Dev Tools & Testing', color: '#a855f7', floatCls: 'dn-f3', Icon: IconTool,
    tools: [
  ...
  {
    category: 'Security Technologies', color: '#f59e0b', floatCls: 'dn-f4', Icon: IconShield,
    tools: [
  ...
  {
    category: 'SEO & Analytics', color: '#E8332A', floatCls: 'dn-f5', Icon: IconSearch,
    tools: [
  ...
  {
    category: 'Design & Infrastructure', color: '#00C8CC', floatCls: 'dn-f6', Icon: IconGlobe,
    tools: [
  ...
  {
    category: 'E-Commerce & Payments', color: '#a855f7', floatCls: 'dn-f7', Icon: IconCart,
    tools: [
  ...
  return (
    <div className="text-center mb-10 sm:mb-14">
      <div className="flex items-center justify-center gap-3 mb-4">
        ...
        <span className="text-[11px] font-bold text-cyan tracking-[0.18em] uppercase font-sans">{label}</span>
        <span className="w-7 h-[2px] bg-cyan" />
      </div>
  ...
  return (
    <div className="font-sans">
      ...
      <section className="bg-navy pt-[70px] relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
          <Reveal>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="dn-sm font-bold text-cyan tracking-[0.18em] uppercase font-sans">What We Offer</span>
            </div>
        ...
        <div className="relative z-10 border-t border-white/8 max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex overflow-x-auto gap-0 scrollbar-hide -mx-4 sm:mx-0 px-4 sm:px-0 flex-nowrap sm:flex-wrap">
            {SERVICES.map((s, i) => (
              <a key={s.slug} href={`#${s.slug}`}
                className="py-3 px-3 sm:px-4 dn-sm font-semibold text-white/40 font-sans border-r border-white/8 hover:text-cyan hover:bg-white/4 transition-all duration-200 whitespace-nowrap flex-shrink-0">
      ...
      {/* ══ SERVICES DETAIL ══ */}
      {SERVICES.map((s, i) => (
        <section key={s.slug} id={s.slug} className={`py-12 sm:py-20 relative overflow-hidden ${i % 2 === 0 ? 'bg-white' : 'bg-[#f5f6f8]'}`}>
              ...
              <div>
                <Reveal>
                  <div className="flex items-center gap-3 mb-3">
                    <span className="w-6 h-[2px] bg-cyan flex-shrink-0" />
                    <span className="dn-sm font-bold text-cyan tracking-[0.15em] uppercase font-sans leading-tight">{s.tagline}</span>
                  </div>
                ...
                {s.perks && s.perks.length > 0 && (
                  <Reveal delay={0.05}>
                    <div className="flex flex-wrap gap-2 mb-5">
                      {s.perks.map(perk => (
                        <span key={perk} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-cyan/10 border border-cyan/30 rounded-lg dn-sm font-bold text-cyan font-sans">
                          ✓ {perk}
                        </span>
                      ))}
                    </div>
                ...
                <Reveal delay={0.15}>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <Link to="/quote" className="px-4 sm:px-5 py-2.5 bg-navy text-white font-bold dn-sm rounded tracking-wide font-sans transition-all duration-300 hover:bg-navy/90 hover:shadow-lg">
                      Get a Free Quote
                    </Link>
                  </div>
                </Reveal>
                  ...
                  <div className="bg-navy rounded-xl p-6 sm:p-10 relative overflow-hidden aspect-square flex flex-col justify-center items-center">
                    ...
                    <div className="relative z-10 text-center">
                      <div className="flex items-end justify-center gap-3 mb-5">
                        <span className="font-serif font-black leading-none" style={{ fontSize: 'clamp(60px,10vw,100px)', lineHeight: 1, color: 'rgba(232,51,42,0.22)' }}>
                          {String(i + 1).padStart(2, '0')}
                        </span>
                        <span className="mb-3 w-8 sm:w-10 h-[2px]" style={{ background: 'rgba(232,51,42,0.4)', display: 'inline-block' }} />
                      </div>
                      <div className="font-serif font-black text-white leading-snug px-4" style={{ fontSize: 'clamp(14px,2.5vw,22px)' }}>
                        {s.title.replace('Affordable ', '')}
                      </div>
                      <div className="inline-block mt-3 px-4 py-2 bg-cyan/10 border border-cyan/20 rounded-lg">
                        <span className="font-serif font-black text-cyan" style={{ fontSize: 'clamp(13px,2vw,18px)' }}>{s.price}</span>
                      </div>
                      {s.perks && s.perks.length > 0 && (
                        <div className="flex flex-wrap justify-center gap-2 mt-4 px-4">
                          {s.perks.map(perk => (
                            <span key={perk} className="px-2.5 py-1 bg-cyan/15 border border-cyan/30 rounded text-[10px] font-bold text-cyan font-sans">✓ {perk}</span>
                          ))}
                        </div>
                      )}
                    </div>
              ...
              <Reveal key={p.step} delay={i * 0.1}>
                <div className="bg-white/4 border border-white/8 rounded-xl p-5 sm:p-8 hover:bg-white/6 transition-all duration-300 h-full">
                  <div className="mb-4 flex items-end gap-2">
                    <span className="font-serif font-black leading-none" style={{ fontSize: 'clamp(44px,7vw,64px)', lineHeight: 1, color: 'rgba(232,51,42,0.25)' }}>{p.step}</span>
                    <span className="mb-3 w-6 h-[2px]" style={{ background: 'rgba(232,51,42,0.4)', display: 'inline-block' }} />
                  </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
            {INDUSTRIES.map((ind, i) => (
              <Reveal key={i} delay={i * 0.04}>
                <div className="bg-[#f5f6f8] rounded-xl p-4 sm:p-5 border border-gray-100 hover:border-cyan/30 hover:bg-cyan/5 hover:shadow-md transition-all duration-300 h-full">
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-cyan/10 border border-cyan/20 flex items-center justify-center mb-3">
                    <span className="w-2.5 h-2.5 rounded-full bg-cyan/70 block" />
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {WHY_US.map((w, i) => (
              <Reveal key={i} delay={i * 0.08}>
                <div className="bg-white/4 border border-white/8 rounded-xl p-5 sm:p-8 hover:bg-white/6 transition-all duration-300 h-full">
                  <div className="mb-4 flex items-end gap-2">
                    <span className="font-serif font-black leading-none" style={{ fontSize: 'clamp(40px,6vw,56px)', lineHeight: 1, color: 'rgba(0,200,204,0.2)' }}>{w.num}</span>
                    <span className="mb-2.5 w-5 h-[2px] bg-cyan/40 inline-block" />
                  </div>
      ...
      <section className="py-12 sm:py-24 bg-white overflow-hidden relative">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6">
          ...
          {/* Tech category cards, 7 cards, responsive grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
            {TECH_CATS.map((cat, i) => (
              <Reveal key={i} delay={i * 0.07}>
                <div className="dn-tc bg-[#f5f6f8] rounded-2xl p-5 sm:p-6 border border-gray-100 hover:border-cyan/30 h-full">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
            {FAQS.map((item, i) => (
              <Reveal key={i} delay={i * 0.05}>
                <div className="bg-white rounded-xl p-5 sm:p-6 border border-gray-100 h-full hover:border-cyan/30 hover:shadow-md transition-all duration-300">
                  <div className="flex items-start gap-3 mb-3">
                    <div className="w-6 h-6 rounded-full bg-cyan/10 border border-cyan/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-cyan text-[10px] font-black">?</span>
                    </div>
          ...
          <Reveal>
            <div className="text-center max-w-2xl mx-auto">
              <div className="flex items-center justify-center gap-3 mb-5">
                <span className="w-7 h-[2px] bg-cyan" />
                <span className="dn-sm font-bold text-cyan tracking-[0.18em] uppercase font-sans">Get Started Today</span>
                <span className="w-7 h-[2px] bg-cyan" />
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Link to="/quote"
                  className="px-6 sm:px-8 py-3.5 sm:py-4 bg-cyan text-navy font-bold dn-sm rounded tracking-wide font-sans transition-all duration-300 hover:bg-cyan/90 hover:shadow-xl hover:shadow-cyan/30 text-center">
                  Get a Free Quote
                </Link>
